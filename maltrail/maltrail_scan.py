...
### 
 ...
 if int(check_malware.returncode) == 0:
  ...
  subprocess.run(
      "sudo cut -d ' ' -f 10 /opt/auto-clamIPS/maltrail/logs/clean.log >> /opt/auto-clamIPS/maltrail/logs/clean2.log", shell=True)
  subprocess.run(
      "sudo sort -u /opt/auto-clamIPS/maltrail/logs/clean2.log >> /opt/auto-clamIPS/maltrail/logs/ipset.log", shell=True)
  subprocess.run(
